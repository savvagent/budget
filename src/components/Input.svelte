...
<div class:hidden={type === 'hidden'} class="w-full relative h-11 text-sm {css}">
	{#if type === 'text'}
		<input
			{autocomplete}
			class="absolute w-full p-2 outline-none border text-sm border-blue-400"
			{id}
			{max}
			{maxLength}
			{min}
	...
	{#if placeholder && !isEmpty(value)}
		<label class="absolute -top-3" for={id}>
			<span class="ml-4 px-1 bg-white text-blue-600">
				{placeholder}
				{#if required}<span class="text-blue-500">*</span>{/if}
			</span>
		</label>
	{/if}
	{#if error}
		<span class="block mt-1 text-xs text-blue-500">{error}</span>
	{/if}
	{#if success}
