<script>
	...
	$: if (emphasis === 'primary') {
		css = `text-white bg-blue-600 hover:bg-blue-500 text-sm cursor-pointer disabled:cursor-not-allowed`;
		...
		css = `bg-transparent shadow-none underline text-blue-500 text-sm cursor-pointer disabled:cursor-not-allowed`;
	} else if (emphasis === 'outline') {
		css = `bg-transparent text-blue-800 border border-blue-500 text-sm cursor-pointer disabled:cursor-not-allowed`;
	} else {
...
{:else}
	<button
		class="flex items-center gap-x-1 px-3 p-2 {css} {klass}"
		{role}
		{type}
		on:click={click}
		{disabled}
